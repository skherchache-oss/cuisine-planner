    ...
    >
      ...
      >
        <div className="flex justify-between items-start mb-2.5 pr-10">
          <span className="font-black uppercase tracking-tight text-[11px] text-gray-900 leading-tight">
            {task.name}
          </span>
          <span className="text-[10px] font-black bg-gray-100 px-2 py-0.5 rounded-lg text-gray-600">
            {format(start, 'HH:mm')}
          </span>
        </div>
        ...
        <div className="flex justify-between items-center text-[10px] font-bold">
          <span className="text-gray-400 truncate max-w-[110px]">👤 {task.responsible}</span>
      ...
      <div className="block md:hidden space-y-6 pb-12">
        <div className="sticky top-[84px] z-40 bg-gray-50/95 backdrop-blur-md pt-2 pb-4 -mx-4 px-4 overflow-x-auto no-scrollbar shadow-sm">
          <div className="flex gap-2 min-w-full">
            {weekDates.map((date, idx) => {
              const dayTasksCount = tasks.filter(t => isSameDay(new Date(t.startTime), date)).length;
        ...
        <div className="px-1 flex items-end justify-between border-b-2 border-gray-100 pb-3">
          ...
          <span className="text-[11px] font-black text-blue-600 bg-blue-50 px-3 py-1 rounded-full uppercase">
            {tasks.filter(t => isSameDay(new Date(t.startTime), weekDates[selectedDayIdx])).length} fiches
          </span>
        </div>
                  ...
                  return (
                    ...
                    >
                      <div className="p-3 flex flex-col gap-3 min-h-[180px]">
                        ...
                        {isOver && (
                          <div className="border-2 border-dashed border-blue-400 bg-blue-100/50 rounded-2xl h-16 animate-pulse" />
                        )}
                        <button onClick={() => onAddTask(dayIdx, shift.id)} className="mt-auto w-full py-2 border-2 border-dashed border-gray-100 rounded-xl text-gray-200 hover:border-blue-200 hover:text-blue-400 transition-all font-black text-2xl">+</button>
                      </div>
                    </td>
                  );
                })}
