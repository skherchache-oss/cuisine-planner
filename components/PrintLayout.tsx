...
const PrintLayout: React.FC<PrintLayoutProps> = ({ tasks, weekLabel, weekStartDate }) => {
  ...
  return (
    <div className="w-full bg-white overflow-visible">
      {/* STYLE INJECTÉ POUR LE PDF : Évite le rognage et force la taille sur Mobile */}
      ...
      >
        ...
        <table className="w-full border-collapse border-2 border-slate-900 table-fixed">
          <thead>
            <tr className="bg-slate-100">
              <th className="border-2 border-slate-900 w-[60px] p-2 text-[10px] font-black uppercase">Shift</th>
              {weekDates.map(date => (
          ...
          <tbody>
            {SHIFTS.map(shift => (
              <tr key={shift.id}>
                <td className="border-2 border-slate-900 bg-slate-50/50 p-2 text-center align-middle">
                ...
                {weekDates.map((date, dayIdx) => {
                  ...
                  return (
                    <td key={dayIdx} className="border-2 border-slate-900 p-1.5 align-top bg-white">
                      <div className="flex flex-col gap-2">
                        {dayTasks.map(task => {
                          ...
                          return (
                            ...
                            >
                              {/* NOM ET HEURE : Texte plus gros et non rogné */}
                              <div className="flex items-start gap-1.5 border-b border-slate-100 pb-1 mb-1">
                                <span className="text-[9px] font-black bg-slate-800 text-white px-1.5 py-0.5 rounded shrink-0">
                              ...
                              {/* RESPONSABLE ET TEMPS */}
                              <div className="flex justify-between text-[9px] font-bold text-slate-600 mb-1">
                                <span className="truncate max-w-[70%]">👤 {task.responsible}</span>
                                <span className="shrink-0 text-slate-900">{task.cookTime}m</span>
                              </div>

                              {/* NOTES / COMMENTAIRES : Affichés si présents */}
                              {task.comments && (
                                <div className="text-[8px] text-slate-700 leading-tight italic bg-slate-50 p-1 rounded mb-1 border-l-2 border-slate-200">
                                  {task.comments}
                                </div>
                              )}

                              {/* DLC : Texte simple pour gagner de l'espace */}
                              <div className="text-[8px] font-bold text-rose-700">
                                DLC: {format(expiry, 'dd/MM HH:mm')}
                              </div>

                              {/* SAISIE MANUELLE : Lignes plus larges */}
                              <div className="flex justify-between items-end gap-1 mt-2">
                                ...
                                <div className="w-8 border-b border-slate-300 h-3 text-center">
        ...
        <div className="mt-4 flex justify-between items-start border-t-2 border-slate-900 pt-2">
          <p className="text-[8px] font-bold text-slate-500 max-w-[60%] leading-tight">
            CONTRÔLE CRITIQUE : TEMPÉRATURES DE CUISSON & DLC OBLIGATOIRES. 
            VALIDATION VISUELLE DU CONDITIONNEMENT.
          </p>
          <div className="text-right border-b border-slate-300 w-32 pb-4">
            <span className="text-[8px] font-black text-slate-400 uppercase">Visa</span>
          </div>
        </div>
      </div>
    </div>
