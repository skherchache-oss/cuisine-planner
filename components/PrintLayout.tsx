...
const PrintLayout: React.FC<PrintLayoutProps> = ({ tasks, weekLabel, weekStartDate }) => {
  ...
  return (
    <div className="bg-white text-black" style={{ width: '297mm', minHeight: '100%' }}>
      
      {/* SECTION 1 : PLANNING HEBDOMADAIRE */}
      <div className="html2pdf__page-break" style={{ padding: '10mm' }}>
        <div className="flex justify-between items-center mb-6 border-b-[5px] border-black pb-4">
          <div className="flex items-center gap-5">
            <div className="bg-black text-white px-5 py-2 text-3xl font-black rounded-xl uppercase tracking-tighter">
              BISTROT M
            </div>
            <div>
            ...
            {SHIFTS.map(shift => (
              <tr key={shift.id}>
                ...
                {weekDates.map((date, dayIdx) => {
                  ...
                  return (
                    <td key={dayIdx} className="border-[2px] border-black p-2 align-top bg-white h-full" style={{ minHeight: '160px' }}>
                      <div className="flex flex-col gap-3">
                        {dayTasks.map(task => {
                          const expiry = calculateExpiry(task.startTime, task.cookTime, task.shelfLifeDays);
                          return (
                            <div key={task.id} className="avoid-break border-[2px] border-black p-2 rounded-md bg-white flex flex-col gap-1.5 shadow-sm">
                              <div className="flex justify-between items-start border-b-[1.5px] border-black pb-1.5 mb-1.5">
                                <span className="font-black uppercase text-[10px] leading-none flex-1 pr-2">{task.name}</span>
                                <span className="font-black text-[9px] bg-black text-white px-1.5 py-0.5 rounded-sm">{format(parseISO(task.startTime), 'HH:mm')}</span>
                              </div>
                              ...
                              <div className="mt-2 pt-2 border-t border-dashed border-black/30">
                                <div className="flex justify-between items-end gap-3">
                                  <div className="flex-1"><div className="text-[7px] font-black uppercase text-gray-400 mb-0.5">T° Cœur</div><div className="border-b-2 border-black h-5"></div></div>
                                  <div className="w-14"><div className="text-[7px] font-black uppercase text-gray-400 mb-0.5">VISA</div><div className="border-b-2 border-black h-5"></div></div>
                                </div>
                              </div>
                            </div>
                          );
      ...
      {/* SECTION 2 : FICHES INDIVIDUELLES (CORRIGÉE POUR LE TEXTE COUPE) */}
            ...
            return (
              ...
              >
                <div className="bg-gray-900 text-white p-4 border-b-[3px] border-black flex justify-between items-center">
                  ...
                  <div className="text-[10px] font-black bg-white/20 px-3 py-1 rounded-full uppercase">
                ...
                <div className="p-5 grid grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4 border-r-2 border-gray-100 pr-6">
                    ...
                    <div className="flex justify-between items-center text-[11px] font-bold border-b border-gray-100 pb-2">
                      <span className="text-gray-400 uppercase tracking-widest">Cuisson</span>
                    ...
                    <div className="bg-red-50 border-2 border-red-100 p-3 mt-4 rounded-xl text-center">
                      <div className="text-[9px] font-black text-red-400 uppercase mb-1 tracking-widest">DLC MAX CALCULÉE</div>
                      <div className="text-sm font-black text-red-900">{format(expiry, 'dd/MM/yyyy HH:mm')}</div>
                    </div>
                  </div>

                  {/* Colonne Droite : Instructions (C'EST ICI QUE CA COUPAIT) */}
                  <div className="text-[11px] flex flex-col h-full">
                    <div className="font-black uppercase text-gray-300 text-[9px] mb-2 tracking-[0.2em]">Procédure & Notes</div>
                    <div className="italic leading-relaxed text-gray-800 bg-gray-50 p-3 rounded-xl border border-gray-100 overflow-visible whitespace-pre-wrap min-h-[120px]">
